/*
 * Handles rigidbodies over the network
 * 
 * Supplied from mpStarTrooper package provided for us
 */
...
public class NetworkRigidbody : MonoBehaviour {
	...
	void OnSerializeNetworkView(BitStream stream, NetworkMessageInfo info) {
		...
		if (stream.isWriting)
		{
			...
			int animState = 0;
			if(animator != null)
				animState = animator.GetInteger("State");
	...
	void Update () {
		...
		if (m_BufferedState[0].timestamp > interpolationTime)
		{
			...
			for (int i=0;i<m_TimestampCount;i++)
			{
				if (m_BufferedState[i].timestamp <= interpolationTime || i == m_TimestampCount-1)
				{
					...
					if(animator != null)
						animator.SetInteger("State", rhs.animState);
		...
		else
		{
			...
			if (extrapolationLength < m_ExtrapolationLimit)
			{
				...
				if(animator != null)
					animator.SetInteger("State", latest.animState);
			}
		}
	}
}
